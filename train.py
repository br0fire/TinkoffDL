...
class ModelText:
        ...
        while i != int(no_time) - 1:
            ...
            for _ in range(self.batch_size):
                if not self.int_text[counter:self.seq_len + counter]:
                    print(
                        "\nError: Слишком маленький датасет и/или некорректные значения seq_len и batch_size в "
                        "ModelText",
                        file=sys.stderr)
                    exit()
    ...
    def fit(self):
        ...
        # self.rnn.embedding.weight.data.copy_(embeddings_matrix)
        # self.rnn.embedding.weight.requires_grad=False
...
def main():
    ...
    parser.add_argument('--input_dir', type=str, help='Путь к датасету')
    parser.add_argument('--model', type=str, default='model.pkl', help='Путь к модели .pkl')
